...
$requirementsPath = Join-Path $mimicKitDir "tools\windows\isaaclab_native_windows_requirements.txt"
$hotfixScript = Join-Path $mimicKitDir "tools\windows\apply_native_isaaclab_hotfixes.py"
$flatdictShim = Join-Path $mimicKitDir "tools\windows\flatdict.py"
...
if (-not $envExists) {
    if ($mambaExe) {
        $createCmd = "`"$mambaExe`" create -y -p `"$CondaPrefix`" python=3.10"
...
Write-Step "Installing native Windows Isaac Lab extras"
Invoke-CondaRun -CondaBat $condaBat -Target $CondaPrefix -UsePrefix -Command "python -m pip install warp-lang --extra-index-url https://pypi.nvidia.com"
Invoke-CondaRun -CondaBat $condaBat -Target $CondaPrefix -UsePrefix -Command "python -m pip install -r `"$requirementsPath`""

Write-Step "Writing flatdict compatibility shim"
$sitePackages = Join-Path $CondaPrefix "Lib\site-packages"
if (-not (Test-Path $sitePackages)) {
    throw "site-packages was not found at $sitePackages"
}
Copy-Item -Force $flatdictShim (Join-Path $sitePackages "flatdict.py")

Write-Step "Accepting Omniverse EULA"
$omniDir = Join-Path $sitePackages "omni"
if (-not (Test-Path $omniDir)) {
    New-Item -ItemType Directory -Force -Path $omniDir | Out-Null
}
Set-Content -Encoding ASCII -Path (Join-Path $omniDir "EULA_ACCEPTED") -Value "YES"

Write-Step "Applying native IsaacLab hotfixes"
Invoke-CondaRun -CondaBat $condaBat -Target $CondaPrefix -UsePrefix -Command "python `"$hotfixScript`" --isaaclab-dir `"$isaacLabDir`""
...
Write-Host "  powershell -ExecutionPolicy Bypass -File `"$mimicKitDir\tools\windows\run_white_knight_mesh_viewmotion.ps1`" -WorkspaceRoot `"$workspaceRoot`" -CondaPrefix `"$CondaPrefix`"" -ForegroundColor Green
try {
