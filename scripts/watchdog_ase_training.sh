...
ROOT_OUT="ase_7case_tmux_20260312_235006"
...
VIZ_CMD="cd ${ROOT_DIR} && ${PYTHON_BIN} tools/ue_bridge/build_mimickit_render_sequences.py --roots ${ROOT_OUT} --frames 300 --frame-stride 5 --no-resume"
...
if [ -f "$OUT_DIR/current_case.txt" ] && grep -q "COMPLETE" "$OUT_DIR/current_case.txt"; then
    if /usr/bin/tmux has-session -t viz_ase 2>/dev/null; then
        if [ ! -f "$VIZ_FLAG" ]; then
            echo "$(date) - Training COMPLETE. Existing viz_ase session found; marking visualization handoff complete." >> "$WATCHDOG_LOG"
            touch "$VIZ_FLAG"
        fi
    elif [ ! -f "$VIZ_FLAG" ]; then
